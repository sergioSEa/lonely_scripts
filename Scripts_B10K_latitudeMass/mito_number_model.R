...
b_overl %>% mutate(Mass_mine=d_overl$unsexed_mass, Estimation=d_overl$`Ratio of Median Depth`,latitude = d_overl$Latitude, tissue= d_overl$`Tissue type`, Order= d_overl$Order) -> b_overl
...
b_overl2 %>% mutate(Mass_mine=d_overl2$unsexed_mass, Estimation=d_overl2$`Ratio of Median Depth`,latitude = d_overl2$Latitude, tissue= d_overl2$`Tissue type`, Order =d_overl2$Order, Other_naming=d_overl2$Latin_name_1) -> b_overl2
...
b_overl3 %>% mutate(Mass_mine=d_overl3$unsexed_mass, Estimation=d_overl3$`Ratio of Median Depth`,latitude = d_overl3$Latitude, tissue= d_overl3$`Tissue type`, Order=d_overl3$Order) -> b_overl3
...
b_overl4 %>% mutate(Mass_mine=d_overl4$unsexed_mass, Estimation=d_overl4$`Ratio of Median Depth`,latitude = d_overl4$Latitude, tissue= d_overl4$`Tissue type`, Order=d_overl4$Order) -> b_overl4
...
b_overl3 %>% mutate("Mass(g)"=`Mean mass (g)`, "BMR(watts)"=`BMR (watts; = 20.1 J/ml O2)`, "Source"="Londono") %>%
  dplyr::select(`Mass(g)`,`BMR(watts)`, Species, Estimation, latitude, Order,Source) %>% arrange(Species) -> overlap_londono
b_overl %>%mutate("BMR(watts)"=BMR_watts, "Source"="Fristoe") %>%  dplyr::select(`Mass(g)`,`BMR(watts)`, Species, Estimation, latitude,Order,Source)%>% arrange(Species) %>% 
  filter(!Species %in% overlap_londono$Species)-> overlap_Fristoe
b_overl4  %>% mutate("BMR(watts)" = conversion(`BMR (ml O2/min)`)*60) -> b_overl4 
b_overl4 %>% mutate("Source"="Bushuev","Mass(g)"=`M (g)`) %>%  dplyr::select(`Mass(g)`,`BMR(watts)`, Species, Estimation, latitude,Order,Source)%>% arrange(Species) %>%
