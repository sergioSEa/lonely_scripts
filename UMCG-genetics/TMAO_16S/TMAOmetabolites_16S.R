...
Fit_model = function(Count_table, Phenotype, Covariates){
...
Calculate_fdr = function(Count_table, Phenotype, Covariates, Real_pvalues){
  FDR_vector = vector()
  ...
  for (i in seq(100)){
    ...
    #Fitmodel
    Fit_model(Rearrange, Phenotype, Covariates) -> H0
    #Save Pvalues in the random distribution of pvalues
    Random_distribution = c(Random_distribution,H0$Pvalue)
  }
