---
title: "Oral microbiome in Periodontitis"
...
In this Markdown document the different steps on the microbiome analysis on Nijmegen oral microbiome project will be presented. The input samples consists on a metadata matrix, a matrix of microbiome relative abundance produced by Metaphlan2, a matrix of pathway abundances produced by humann2 and a file with statistics on human contamination and number of reads (based on kneadata output).

Let us begin by loading the packages needed and setting up the work location where the files are located.
I use tidyverse mainly to use function from dplyr, ggplot and readr, for easy data handling. Vegan and ape are phylogenetic and ecological packages that are widely used in microbiome analysis. Ggforce extends some functions in ggplot. Ggtree is a ggplot-based tree visualizing tool. Cluster allows to cluster the data. DT allows for interactive tables to be displayed.
...
Next I will load the data using readr functions. The taxonomy matrix contains information on different taxonomic levels encoded as a letter followed by two underscroes. It also contains a second header that I will remove.
...
"do_bars", generates a bar plot comparing the taxonomic relative abundance betweeen the two tissues, and between patients and controls. It also generates and outputs a median abundance per species in Controls, Cases, Pocket and Saliva.
...
We transform the data by means of the arg sin square transformation. This tries to make the abundances of a particular bugg to be normally distributed. Then, we remove all the bugs that are not present in at least 20% of the samples (we don't want to test for samples with really small power). Then we fit three different models. One with all data, other with Pocket and other with Saliva. In the two later cases I record the p-values of the Group (Cases, Controls) and the p-values of a test for normally, since the linear model assumes that the distribution of residuals is normally distributed. For the first case (where both pocket and saliva are together) I recorded the p-value of the Source (Saliva/Pocket) and controlled for Group and number of reads.
P-values for normality and effect where corrected with FDR.
...
Differential_abundance = function(taxa_matrix,metadata, transformation = "argsinsquare"){
  ...
  Model_1 = fit_linear_model(abundance_matrix, metadata, "source", "Source")
  ...
  return(list(Model_1,Model_2,Model_3))
...
We do the same process but with the different Pathway abundances
...
This are functions called in some of the functions described above.
...
Make absolute abundance to relative
...
## Data Analysis
Once the functions are ready, we can start the analysis.
...
As you can see, the distribution of contamination has a median around 20% of contamination and the more reads that were found the higher the contamination is. What makes sense, we have more reads because there was more human material to start with.
...
This shows some effect on the number of reads.



For comparing the composition, we go to the family level or above, since there are many Genuses or Species
...
We show the family compositon when comparing two different tissues and when comparing the two groups. But this is hard to visualize, because there are several. Thus it is easier to check it as a table


```{r Top composition}
DT::datatable(List_tops)
```


We can also look for the median Species (instead of Families) at each condition
...
You might check that table for specific Taxas in each of the groups of interest (Saliva/pocket/Cases/Controls)

As we might be interested on looking at the top species only on pocket, we can write some code since the function doesn't do this.
...
combined_matrix %>% filter(Source=="pocket" && as.character(Group)=="0") %>% select(colnames(taxa_matrix)) %>% summarise_all(median) %>%
    gather(Taxa, Median_relative_abundance, 1:length(colnames(taxa_matrix)), factor_key=TRUE) %>%
  arrange(desc(Median_relative_abundance))  %>% mutate(TOP="Healthy") -> top
combined_matrix %>% filter(Source=="pocket")%>% filter(Group==1) %>% select(colnames(taxa_matrix)) %>% summarise_all(median) %>%
    gather(Taxa, Median_relative_abundance, 1:length(colnames(taxa_matrix)), factor_key=TRUE) %>%
  arrange(desc(Median_relative_abundance)) %>%  mutate(TOP="Patients") -> top2
...
And the same goes for family composition on pocket samples only
...
combined_matrix %>% filter(Source=="pocket") %>% gather(Taxa, Abundance, (dim(sample_metadata)[2]+1):dim(combined_matrix)[2], factor_key=TRUE) %>%
  ggplot() + geom_bar(aes(x=StudieID,y=Abundance, fill=Taxa), stat = "identity",position="stack") + facet_wrap(~Group, dir="v", scales="free")+
   theme_bw() + theme(axis.text.x = element_text(angle = 90), legend.position = "bottom",legend.title = element_text(size = 3), legend.text = element_text(size = 6))+
    theme(legend.key.size = unit(0.1, "cm")) -> BAR1
print(BAR1)
```
...
On the species level, the paired Wilcoxon test did not yield significant differences between Cases and Controls in Pocket, Saliva or combined samples. Meaning that there is no statistical assurance of differences in Micobial diversity in both Groups.
Nevertheless, when we look at the clustering of the samples, both the PCoA and the hierarchical cluster seem to point to some clustering based on the Group (Cases/Controls) on the Pocket samples. The saliva samples seem more similar among them (PCoA plot). This is statistically confirmed in the MANOVA test. MANOVA allow us to test the effect of a dependent variable and covariates in all the bugs at the same time and is a powerful test.

Let's check what are the trends observed in Families
...
Similar findings can be seen here as the ones described for Species.


### Differential Abundance 

Differences on individual taxas in Species and Families can be interrogated when comparing Cases and Controls. We have two major issues here. Fist is that the bugg abundance distribution is not normal and has a great amount of 0s. Linear modelling assumes that the residuals of the model are normally distributed,this is normally associated with the distribution of the dependent variable. In the results I give both the p-value of the Beta of interest (Source or Group depending on the test) and a p-value of the distribution of the residuals being normal. This means that in an ideal scenario we would like the p-value of the Beta being significantly low while the p-value of the normality not being significantly low. The second problem, is that the amount of test is high, and only highly significant p-values will remain significant after correcting for multiple testing. Therefore, we try to make fewer tests by removing bugs where there is not a prevalence on at least 20% of the samples.

Three models are performed, one for the bugg in pocket, other for the bugg in saliva and finally a combination of slaiva and pocket, in which case I interrogate if we see differences between the sample source (And not the Group).



```{r Individual differential Species abundance, echo = F}
Differential_abundance(species_matrix2,sample_metadata,"argsinsquare") -> Results

```


Species differential abundance between Saliva and Pocket
```{r  Differential Species abundance Saliva and Pocket}
Results[[1]] %>% arrange(FDR) %>% DT::datatable()
```

Species differential abundance between Cases and Controls in Pocket

```{r  Differential Species abundance Cases and Controls Pocket}
Results[[2]] %>% arrange(FDR) %>% DT::datatable()
```

Species differential abundance between Cases and Controls in Saliva

```{r  Differential Species abundance Cases and Controls Saliva}
Results[[3]] %>% arrange(FDR) %>% DT::datatable()
```


No significant hits were found between Cases and Controls. In the combined set, there are significant heats, meaning that the bugg has a significant abundance in Saliva and Pocket. Nevertheless the nromality P-values are really low, meaning that the distribution of residuals is hardly normal. This breaks the assumption of the lineal model and the results might not be accurate.

Studying the same on the family level


```{r Individual differential Family abundance, echo = F}
Differential_abundance(family_matrix2,sample_metadata,"argsinsquare") -> Results
```

Family differential abundance between Saliva and Pocket
```{r  Differential Family abundance Saliva and Pocket}
Results[[1]] %>% arrange(FDR) %>% DT::datatable()
```

Family differential abundance between Cases and Controls in Pocket

```{r  Differential Family abundance Cases and Controls Pocket}
Results[[2]] %>% arrange(FDR) %>% DT::datatable()
```

Family differential abundance between Cases and Controls in Saliva

```{r  Differential Family abundance Cases and Controls Saliva}
Results[[3]] %>% arrange(FDR) %>% DT::datatable()
```


We find one signifcant hit on Saliva, where there is a significant difference in abundances bewteen Controls and Cases.
...
DT::datatable(species_control)
```
And in Cases
```{r Most abundant in pocket3}
DT::datatable(species_patient)
```
Biggest fold changes
```{r Most abundant in pocket4}
DT::datatable(Fold_changes)
```
...
Differential Pathway abundance between both tissue source

```{r Pathway abundance output1}
DT::datatable(pathway_results[[1]])
```

Differential Pathway abundance in Pocket between Controls and Cases

```{r Pathway abundance output2}
DT::datatable(pathway_results[[2]])
```

Differential Pathway abundance in Saliva between Controls and Cases

```{r Pathway abundance output3}
DT::datatable(pathway_results[[3]])
```
