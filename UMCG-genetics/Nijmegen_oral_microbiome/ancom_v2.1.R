...
ANCOM = function(feature_table, meta_data, struc_zero = NULL, main_var, p_adj_method = "BH", 
  ...
  # Draw volcano plot
  # Calculate clr
  Pseudocount = as.vector(feature_table)[! as.vector(feature_table) == 0]
  Pseudocount = min(Pseudocount)/2
  ...
  meta_data %>% pull(main_var) -> fucking_x
  PLOTS = apply(clr_table, 1, function(y){ ggplot() + geom_boxplot(aes(x=fucking_x, y=y)) + geom_point(aes(x=fucking_x, y=y)) })
  ...
  res = list(out = out, fig = dat_fig, Plots = PLOTS)
  return(res)
}
